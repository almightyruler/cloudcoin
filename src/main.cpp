...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    // ppcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        ...
        if (!fTestNet && Checkpoints::hashSyncCheckpoint != uint256("0x000001308acad354c7768a7a8ebee2e5d61743d624dcce73da3dee5b599bebb5")) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->strSubVer == "/Satoshi:0.7.2/" && pfrom->nStartingHeight < 285000)  
        {
            // CDC v1.3: reject connections with original clients (0.7.2) which appear to be on the old chain. 285000 is a semi arbitrary number, chosen
            // to leave a reasonable test gap between the old and correct chain. eventually the old chain could reach this number and the conditional
            // will fail, but hopefully by then everyone will have upgraded.
            printf("partner %s using original client and possibly on old chain; disconnecting\n", pfrom->addr.ToString().c_str() );
            pfrom->fDisconnect = true;
            return false;
        }
